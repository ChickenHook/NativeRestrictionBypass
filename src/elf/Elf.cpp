...
namespace ElfParser {
     ...
     */
    bool Elf::isElf(const char *addr) {
        if (addr == nullptr) {
            log("addr is null");
            return false;
        }
//        HexDump::hex_dump(addr, magic.size(), std::cout);

        int i = 0;
        for (auto b:ELF_MAGIC) {
            if (b != addr[i]) {
                log("Magic doesn't match elf (%d) <%x> : <%x>", i, b, addr[i]);
                return false;
            }
            i++;
        }
     ...
     */
    bool Elf::init(const char *addr) {
        this->addr = addr;
        if (is64()) {
            _elf_parser = std::make_unique<Elf64Parser>(addr);
        } else {
    ...
    uint32_t Elf::getArchitecture() {
        if (is64()) {
            auto *header = (Elf64_Ehdr *) &addr[0];
            return header->e_machine;
        } else {
            auto *header = (Elf32_Ehdr *) &addr[0];
            return header->e_machine;
        }
    }
...
//        init(nullptr);
    ...
    bool Elf::iterateSymbolTable(const std::vector<int> &types,
                                 const std::function<bool(int, char *, uint64_t,
                                                          uint8_t)> function) {
        return _elf_parser->iterateSymbolTable(types, function);
    }

    bool Elf::IterateRelocationTable(const std::vector<int> &types,
                                     const std::function<bool(int, char *, uint64_t,
                                                          uint8_t)> function) {
        return _elf_parser->IterateRelocationTable(types, function);
    }
}